...
export function ProductCartCard({ product, quantity }: CartProductType): JSX.Element {
  return (
    <div data-id={product.id} className="py-2 px-2 flex gap-3">
      <Link className="aspect-square w-16 relative" to={`/catalog/${product.id}`}>
        <div className="absolute right-1 -top-1 rounded-full w-5 h-5 flex items-center justify-center
        text-[0.75rem] 
        dark:bg-brand-dark dark:text-white">
          {quantity}
        </div>
        <ProductImage imageUrl={product.imageUrl} altText={product.name} isSmall={true} />
      </Link>
      <div className="w-full flex justify-between items-center">
        <div>
